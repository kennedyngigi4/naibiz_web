...
export default function Page() {
    ...
    const slug: any = params.professionalSlug;
    ...
    useEffect(() => {
        const fetchData = async() => {
            const res = await APIServices.get(`professional/professional/${slug}/`);
            console.log(res);
    ...
    return (
        <>
            ...
            <section className="bg-cover position-relative ht-500 py-0" style={{ backgroundImage: `url(${professionalData && professionalData.banner_image}` }} data-overlay="4">
                                        ...
                                        <div className="listingFirstinfo d-flex align-items-center justify-content-start gap-3 flex-wrap">
                                            <div className="listingAvatar">
                                                {professionalData?.profile_image  ? (
                                                    <Link href="#" className="d-block"><Image src={professionalData && professionalData?.profile_image} width={95} height={95} className="img-fluid rounded-3" alt={professionalData?.fullname || "Nairobi Business Listing"} /></Link>
                                                ) : (
                                                    <div className="d-flex align-items-center justify-content-center w-100 h-100">
                                                        <Image src="/icons/user.png" width={0} height={0} sizes='100vw' style={{ width: '100%', height: '100%' }} className="img-fluid circle" alt={professionalData?.fullname || "Nairobi Business Listing"} />
                                                    </div>
                                                )}
                                            </div>
                                            <div className="listingCaptioninfo">
                                                <div className="propertyTitlename d-flex align-items-center gap-2 mb-1"><h2 className="fw-semibold text-light mb-0">{professionalData && professionalData?.fullname}</h2><span className="verified mt-1"><img src='/img/tick.svg' className="img-fluid" width="22" alt={professionalData?.fullname} /></span></div>
                                                <div className="listingsbasicInfo">
                                                    <div className="d-flex align-items-center justify-content-start flex-wrap gap-2">
                                                        <div className="flexItem me-2"><span className="text-md fw-medium text-light d-flex align-items-center"><FaLocationDot className="me-2" />{professionalData && professionalData.location}</span></div>
